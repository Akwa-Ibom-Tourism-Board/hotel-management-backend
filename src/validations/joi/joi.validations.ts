...
const hotelSchema = Joi.object({
  ...
  // FIX: custom validator strips markdown link format and normalises URL
  website: Joi.when("hasWebsite", {
    is: true,
    then: Joi.string()
      .trim()
      .custom((value, helpers) => {
        ...
        const cleanUrl = markdownMatch ? markdownMatch[1] : value;
  ...
  // FIX: accept any string so custom "Other" values pass validation
  facilities: Joi.array()
    .items(Joi.string())
    .default([]),

  facilitiesOther: Joi.string().trim().max(200).allow("", null).optional(),
  ...
  seatingCapacity: Joi.when("entityType", {
    ...
    then: Joi.number().integer().min(1).required().messages({
      ...
      "number.min": "Seating capacity must be at least 1",
      "any.required":
        "Seating capacity is required for restaurants, bars, and lounges",
    }),
    otherwise: Joi.number().integer().min(1).allow(null).optional(),
  }),
  ...
  serviceTypesOther: Joi.string().trim().max(200).allow("", null).optional(),
...
// FIX: accepts both a single flat entity and { establishments: [...] }
const singleOrBulkHotelSchema = Joi.alternatives()
  .try(bulkHotelSchema, hotelSchema)
  .messages({
    "alternatives.match": "Invalid establishment data provided",
  });
